/*
    ...
    Copyright (C) 2001-2026  Ludovic Rousseau <ludovic.rousseau@free.fr>
...
static void spin_stop(void)
{
	...
	/* unlock the pthread_cond_timedwait() */
	pthread_cond_signal(&spinner_cond);
}
...
static void print_version(void)
{
	...
	printf("V %s (c) 2001-2026, Ludovic Rousseau <ludovic.rousseau@free.fr>\n",
			PACKAGE_VERSION);
}
	...
	if (SCARD_E_NO_READERS_AVAILABLE == rv || 0 == nbReaders)
	{
		...
		else
		{
			...
			while ((SCARD_S_SUCCESS == rv) && (dwReaders == dwReadersOld))
			{
				...
				/* wait before next SCardListReaders() call */
				sleep(1);
